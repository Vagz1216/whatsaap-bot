...
*   `NEXT_PUBLIC_CLERK_PUBLISHABLE_KEY`: Clerk browser key used by the static dashboard to render sign-in.
*   `PLATFORM_OWNER_EMAILS`: Comma-separated emails that should receive platform owner access.

The backend validates Clerk JWTs and the static dashboard uses `NEXT_PUBLIC_CLERK_PUBLISHABLE_KEY` to create Clerk browser sessions. `DASHBOARD_TOKEN` is only a local/internal fallback when `CLERK_JWKS_URL` is empty. Do not use it as the main production auth mechanism.
